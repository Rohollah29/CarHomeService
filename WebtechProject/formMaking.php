...
<body>
    ...
    <form method="POST" autocomplete="on" action="dbinfo.php">
        ...
        <fieldset>
            ...
            <input type="text" id="first_name" name="c" autocomplete="given-name"><br><br>
    ...
    <script>
        document.querySelector("form").addEventListener("submit", function(e) {
            ...
            if (!document.getElementById("father_name").value.trim()) errors.push("Father's Name is required.");
            if (!document.getElementById("dob").value.trim()) errors.push("Date of Birth is required.");
            if (!document.querySelector("input[name='gender']:checked")) errors.push("Gender is required.");
            if (!document.getElementById("email").value.trim()) errors.push("Email is required.");
            if (!document.getElementById("phone").value.trim()) errors.push("Phone Number is required.");

            // Address Info
            if (!document.getElementById("post_office").value.trim()) errors.push("Post Office is required.");
            if (!document.getElementById("postal_code").value.trim()) errors.push("Postal Code is required.");
            if (!document.getElementById("thana").value) errors.push("Thana is required.");
            if (!document.getElementById("district").value) errors.push("District is required.");
            if (!document.getElementById("division").value) errors.push("Division is required.");
            if (!document.getElementById("current_address").value.trim()) errors.push("Current Address is required.");
            if (!document.getElementById("permanent_address").value.trim()) errors.push("Permanent Address is required.");

            // Vehicle Info
            if (!document.getElementById("car_brand").value) errors.push("Car Brand is required.");
            if (!document.getElementById("license_number").value.trim()) errors.push("License Number is required.");
            if (!document.getElementById("car_one_color").value) errors.push("Car One Color is required.");
            if (!document.getElementById("car_service_count").value.trim()) errors.push("Car Service Count is required.");
